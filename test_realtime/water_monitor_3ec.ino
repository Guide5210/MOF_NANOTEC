...
// ============================================================================
/*
 * หัววัดที่ไม่ตอบติดกันหลายครั้ง จะถูกพักไว้ชั่วคราว
 *
 * ModbusMaster รอคำตอบนานถึง 2 วินาทีต่อหนึ่งครั้งที่ไม่มีใครตอบ และ
 * ระหว่างที่รอ มันจะอ่านไบต์ทุกอย่างบนบัสทิ้ง รวมถึงเฟรมคำสั่งที่จอส่งมา
 * ผลคือพอถอดหัววัดออกหนึ่งตัว คำสั่งคาลิเบรตจากจอจะหายไปเกือบทุกครั้ง
 *
 * การพักตัวที่ตายแล้วจึงแก้สองอย่างพร้อมกัน: คืนช่วงเวลาให้บัสว่างพอที่
 * คำสั่งจะเข้ามาได้ และทำให้รอบการอ่านตัวที่ยังดีอยู่เร็วขึ้นมาก
 */
#define DEAD_AFTER_FAILS   3
#define DEAD_SKIP_CYCLES   8

uint8_t failStreak[N_SENSORS] = {0};
uint8_t skipCycles[N_SENSORS] = {0};

/*
 * ยิงคำถามแล้วไม่รอคำตอบนาน ใช้กับหัววัดที่รู้อยู่แล้วว่าไม่ตอบ
 *
 * ⚠️ ต้องยิงออกบัสจริง ห้ามข้ามเงียบ ๆ
 *    จอใช้ "คำถามถึงหัววัดตัวที่ 1" เป็นสัญญาณว่าขึ้นรอบใหม่ แล้วปิดรอบเก่า
 *    ส่งข้อมูลออก ถ้าเราเงียบไป จอจะไม่ปิดรอบ ข้อมูลค้างจนขึ้น STALE
 *    ทั้งที่หัววัดตัวอื่นยังอ่านได้ปกติ
 *
 *    ต่างจากการเรียก ModbusMaster ตรงที่ไม่รอ 2 วินาที บัสจึงว่างพอให้
 *    คำสั่งจากจอแทรกเข้ามาได้
 */
void probeQuick(int i) {
  uint8_t req[8] = { ADDRS[i], 0x03, 0x00, 0x00, 0x00, 0x02, 0, 0 };
  ...
  uint32_t t0 = millis();
  while (millis() - t0 < 120) {
    if (RS485Serial.available()) RS485Serial.read();
  }
}
...
void readOne(int i) {
  if (skipCycles[i] > 0) {
    ...
    okVal[i] = false;
    errCount[i]++;
    probeQuick(i);                /* ยังยิงคำถาม แต่ไม่รอคำตอบนาน */
    return;
  }
  ...
  if (r == node.ku8MBSuccess) {
    ...
    okVal[i] = false;
    errCount[i]++;
    if (failStreak[i] < 255) failStreak[i]++;
    if (failStreak[i] >= DEAD_AFTER_FAILS) skipCycles[i] = DEAD_SKIP_CYCLES;
    delay(MB_GAP_MS);
    return;
  }
  delay(MB_GAP_MS);
...
void handleSerialCmd() {
  ...
  while (Serial.available()) {
    ...
    if (c == '\n') {
      ...
      // Z -> กวาดหา address ของหัววัดที่หายไป
      //
      // ใช้ตอนหัววัดตัวหนึ่งเงียบทั้งที่ตัวอื่นบนสายเดียวกันตอบปกติ
      // ถ้ามันแค่ถูกเขียน address ทับไป จะเจอในรอบนี้และกู้ได้
      //
      // ต้องให้บอร์ดนี้เป็นคนกวาด เพราะตัวขับ RS485 ของจอดันบัสที่มีโหลด
      // หนักไม่ขึ้น กวาดจากฝั่งจอเลยไม่ได้ผล
      //
      // ส่งเฟรมดิบเองแทน ModbusMaster เพราะไลบรารีตั้ง timeout ไว้ 2 วินาที
      // ต่อหนึ่ง address ซึ่งจะกินเวลาทั้งรอบเกิน 8 นาที
      if (buf[0] == 'Z' || buf[0] == 'z') {
        Serial.println(F("[scan] กวาด address 1-247 ..."));
        int hits = 0;
        for (int a = 1; a <= 247; a++) {
          uint8_t req[8] = { (uint8_t)a, 0x03, 0x00, 0x00, 0x00, 0x02, 0, 0 };
          uint16_t c = mbCRC(req, 6);
          req[6] = (uint8_t)(c & 0xFF);
          req[7] = (uint8_t)(c >> 8);
          ...
          while (millis() - t0 < 300 && n < 9) {
            if (RS485Serial.available()) rsp[n++] = RS485Serial.read();
          }
          if (n == 9 && rsp[0] == (uint8_t)a && rsp[1] == 0x03 &&
              mbCRC(rsp, 7) == (uint16_t)(rsp[7] | (rsp[8] << 8))) {
            uint16_t ec = ((uint16_t)rsp[3] << 8) | rsp[4];
            Serial.print(F("[scan] เจอ address "));
            Serial.print(a);
            Serial.print(F("  EC="));
            Serial.println(ec / 10.0f);
            hits++;
          }
          else if (n > 0) {
            /* มีไบต์กลับมาแต่ไม่ผ่านเงื่อนไข — โชว์ดิบไว้ดูว่าเพี้ยนตรงไหน */
            Serial.print(F("[scan] addr "));
            Serial.print(a);
            Serial.print(F(" ได้ "));
            Serial.print(n);
            Serial.print(F(" ไบต์:"));
            for (int k = 0; k < n; k++) {
              Serial.print(' ');
              if (rsp[k] < 0x10) Serial.print('0');
              Serial.print(rsp[k], HEX);
            }
            Serial.println();
          }
          if ((a % 25) == 0) { Serial.print(F("[scan] .. ")); Serial.println(a); }
          delay(MB_GAP_MS);
        }
        Serial.print(F("[scan] จบ พบ "));
        Serial.print(hits);
...
void loop() {
  ...
  if (now - tPoll >= POLL_EVERY_MS) {
    tPoll = now;
    /* แทรกการรับคำสั่งระหว่างการอ่านแต่ละตัว ไม่ใช่แค่ต้นลูป
     * ยิ่งมีจังหวะเช็คถี่ โอกาสที่เฟรมคำสั่งจะถูกกลืนก็ยิ่งน้อย */
    for (int i = 0; i < N_SENSORS; i++) {
      pollBusCommands();
      readOne(i);
    }
